...
SET TECH_TYPE /CONDENSING,CHP-BACK-PRESSURE,CHP-EXTRACTION,BOILERS,ELECT-TO-HEAT,INTERSEASONAL-HEAT-STORAGE,INTERSEASONAL-ELECT-STORAGE,INTRASEASONAL-HEAT-STORAGE,INTRASEASONAL-ELECT-STORAGE,HYDRO-RESERVOIRS,HYDRO-RUN-OF-RIVER,WIND-ON,WIND-OFF,SOLAR-PV,SOLAR-HEATING,HYDRO-WAVE,HEAT-PUMP,EL-BOILER,FUELCELL,ELECTROLYZER,H2-STORAGE,BIOMETH-DAC,HUB-OFF,STEAMREFORMING/;
SET GTECH_TYPE(GGG,TECH_TYPE);
SET PRICE_CATEGORY /AVERAGE,AVERAGE_WEIGHTED_BY_CONSUMPTION,AVERAGE_WEIGHTED_BY_PRODUCTION/;
SET VARIABLE_CATEGORY /EXOGENOUS,ENDOGENOUS,DECOMMISSIONING,ENDOGENOUS_ELECT2HEAT,ENDO_INTRASTO,ENDO_INTERSTO,ENDO_EV,ENDO_HEATPUMP,ENDO_ELBOILER,ENDO_OTHERTRANS,DIST_LOSSES,TRANS_LOSSES,ENDO_CCS,ENDO_H2,ENDO_BIOMETHANE,ENDO_FUELCELL,ENDO_ABSCHILLER/;
...
SET H_BAL_TYPE /CURTAILMENT,NETEXPORT,PRICE,DEMAND_EXO,DEMAND_LOSS,DEMAND_INTERSTO,DEMAND_INTRASTO,BOILERS,CHP-BACK-PRESSURE,CHP-EXTRACTION,INTER-STO,INTRA-STO,SOLAR-HEATING,P2H,FUELCELL,DEMAND_P2G,DEMAND_DISTLOSSES,DEMAND_ABSCHILLER/;
...
PARAMETER  BIOMETH_PRICE_YST(YYY,SSS,TTT,UNITS) "Hourly biomethane Prices for each region (money/MWh)";
execute_load  '../../base/auxils/timestep_conversion/input/MainResults.gdx', BIOMETH_PRICE_YST;
PARAMETER XH2_FLOW_YCRST(YYY,CCC,IRRRE,IRRRI,SSS,TTT,UNITS) "H2 Transmission flow for each year, country, from region to region, for each season and hour (MWh)";
execute_load  '../../base/auxils/timestep_conversion/input/MainResults.gdx', XH2_FLOW_YCRST;
PARAMETER XH2_CAP_YCR(YYY,CCC,IRRRE,IRRRI,VARIABLE_CATEGORY,UNITS) "H2 Transmission capacity for each year, country, from region to region (GW)";
execute_load  '../../base/auxils/timestep_conversion/input/MainResults.gdx', XH2_CAP_YCR;
PARAMETER XH2_FLOW_YCR(YYY,CCC,IRRRE,IRRRI,UNITS) "H2 Transmission flow for each year, country, from region to region (TWh)";
execute_load  '../../base/auxils/timestep_conversion/input/MainResults.gdx', XH2_FLOW_YCR;
PARAMETER ECO_XH2_YCR(YYY,CCC,RRR,IRRRI,CATEGORY,SUBCATEGORY,UNITS) "H2 Transmission Economic Output for each year, country and region (Mmoney)";
execute_load  '../../base/auxils/timestep_conversion/input/MainResults.gdx', ECO_XH2_YCR;
PARAMETER H2_DEMAND_YCRST(YYY,CCC,RRR,SSS,TTT,VARIABLE_CATEGORY,UNITS) "Aggregated Hourly H2 Demand (MWh)";
execute_load  '../../base/auxils/timestep_conversion/input/MainResults.gdx', H2_DEMAND_YCRST;
PARAMETER H2_DEMAND_YCR(YYY,CCC,RRR,VARIABLE_CATEGORY,UNITS) "Aggregated annual H2 Demand (TWh)";
execute_load  '../../base/auxils/timestep_conversion/input/MainResults.gdx', H2_DEMAND_YCR;
...
PARAMETER BIOMETH_PRICE_YST_NEW(YYY,SSS_NEW,TTT_NEW,UNITS) "Hourly biomethane Prices for each region (money/MWh)";
PARAMETER XH2_FLOW_YCRST_NEW(YYY,CCC,IRRRE,IRRRI,SSS_NEW,TTT_NEW,UNITS) "H2 Transmission flow for each year, country, from region to region, for each season and hour (MWh)";
PARAMETER H2_DEMAND_YCRST_NEW(YYY,CCC,RRR,SSS_NEW,TTT_NEW,VARIABLE_CATEGORY,UNITS) "Aggregated Hourly H2 Demand (MWh)";
...
BIOMETH_PRICE_YST_NEW(YYY,SSS_NEW,TTT_NEW,UNITS)=SUM((SSS,TTT)$ST_LINK(SSS,TTT,SSS_NEW,TTT_NEW),BIOMETH_PRICE_YST(YYY,SSS,TTT,UNITS))*WEIGHT_T_NEW(TTT_NEW);
XH2_FLOW_YCRST_NEW(YYY,CCC,IRRRE,IRRRI,SSS_NEW,TTT_NEW,UNITS)=SUM((SSS,TTT)$ST_LINK(SSS,TTT,SSS_NEW,TTT_NEW),XH2_FLOW_YCRST(YYY,CCC,IRRRE,IRRRI,SSS,TTT,UNITS));
H2_DEMAND_YCRST_NEW(YYY,CCC,RRR,SSS_NEW,TTT_NEW,VARIABLE_CATEGORY,UNITS)=SUM((SSS,TTT)$ST_LINK(SSS,TTT,SSS_NEW,TTT_NEW),H2_DEMAND_YCRST(YYY,CCC,RRR,SSS,TTT,VARIABLE_CATEGORY,UNITS));
...
BIOMETH_PRICE_YST_NEW=BIOMETH_PRICE_YST,
ECO_XH_YCRA,
XH2_FLOW_YCRST_NEW=XH2_FLOW_YCRST,
XH2_CAP_YCR,
XH2_FLOW_YCR,
ECO_XH2_YCR,
H2_DEMAND_YCRST_NEW=H2_DEMAND_YCRST
