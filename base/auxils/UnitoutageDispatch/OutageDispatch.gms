...
*Hydrogen addon
ACRONYM HYDROGEN_GH2STO, H2_STORAGE, HYDROGEN_GETOH2,HYDROGEN_GETOHH2,HYDROGEN_GEHTOH2, HYDROGEN_GCH4TOH2, HYDROGEN_GH2FUEL, HYDROGEN_GH2TOE, HYDROGEN_GH2TOEH, HYDROGEN_GH2TOBIOMETH, FUELCELL, BIOMETHDAC, ELECTROLYZER,BIOGAS_H2,THERMGAS_H2,BIOMETHANE,HYDROGEN;
...
LOOP(IA,
  LOOP(G$(GKFX(Y,IA,G) AND GDATA(G,'GDFOR')),
    ...
    IF(GKFX(Y,IA,G)/GDATA(G,'GDUCUNITSIZE') LT 1,
    IGKRATE2(IA,G,S,T)=0;
    );
    IF(GKFX(Y,IA,G)/GDATA(G,'GDUCUNITSIZE') GE 1,
    IGKFXSAVAIL_NET = GKFX(Y,IA,G)-UCONMAINT(Y,IA,G,S)*GDATA(G,'GDUCUNITSIZE');
    ...
    IGKRATE(IA,G,S,T)$IGKFXSAVAIL_NET= (IGKRATE(IA,G,S,T) + 1$(NOT IGKRATE(IA,G,S,T)))*(IGKFXSAVAIL/IGKFXSAVAIL_NET);
    IGKRATE(IA,G,S,T)$(IGKRATE(IA,G,S,T) LE 0) = EPS;
