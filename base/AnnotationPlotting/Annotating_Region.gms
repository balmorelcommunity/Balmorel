...
QEEQ.stage(IR,S,T) = card(blocks) + 2;

*Intra-seasonal heat storage dynamic equation (MWh)
QHSTOVOLTS.stage(IA,S,T) = card(blocks) + 2;

* Heat generation equals demand (MW)
QHEQ.stage(IA,S,T) = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;

* Fuel consumption rate.
QGFEQ.stage(IA,G,S,T) = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;
* CHP generation (back pressure) limited by Cb-line (MW)
QGCBGBPR.stage(IA,G,S,T) = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;

* CHP generation (extraction) limited by Cb-line (MW)
QGCBGEXT.stage(IA,G,S,T) = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;

* CHP generation (extraction) limited by Cv-line (MW)
QGCVGEXT.stage(IA,G,S,T) = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;

* Electric heat generation (MW)
QGGETOH.stage(IA,G,S,T) = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;
...
*QGFRAMPUP.stage(IA,G,S,T) = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;

* Maximum fuel rampdown for exogenous capacities (MWh)
*QGFRAMPDOWN.stage(IA,G,S,T) = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;


* For instance 2
QHYRSSEQ.stage(IA,S) = card(blocks) + 2;

QHYRSMAXVOL.stage(IA,S) = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;
...
VQHSTOVOLTS.stage(IA,S,T,IPLUSMINUS)  = 1;


* Electricity generation (MW), existing units
VGE_T.stage(IA,G,S,T) = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;
* Heat generation (MW), existing units

*** NOT IN THE SAME BLOCK!
VGH_T.stage(IA,G,S,T) = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;
* Fuel consumption rate (MW), existing units
VGF_T.stage(IA,G,S,T) = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;
...
*VHSTOLOADT.stage(IA,S,T)  = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;
VHSTOLOADTS.stage(IA,S,T)  = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;

* Feasibility in electricity balance equation QEEQ (MW)
VQEEQ.stage(IR,S,T,IPLUSMINUS)   = sum(blockAssignmentRegions(blocks,IR),ord(blocks)) + 1;

* Feasibility in heat balance equation QHEQ (MW)
VQHEQ.stage(IA,S,T,IPLUSMINUS)  = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;
...
* Instance 2 extra
VHYRS_S.stage(IA,S) = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;
VQHYRSSEQ.stage(IA,S,IPLUSMINUS) = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;
VQHYRSMINMAXVOL.stage(IA,S,IPLUSMINUS)  = sum(blockAssignmentAreas(blocks,IA),ord(blocks)) + 1;
